...
export const GroupSettingsModal = () => {
    ...
    useEffect(() => {
        if (conversation) {
            ...
            form.setValue("imageUrl", conversation?.image || "")
        }
    }, [conversation, form]);
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        try {
            await axios.patch(`/api/group`, {values,conversationId: conversation?.id})
            router.refresh();
            onClose();
    ...
    return (
        <Modal isOpen={isModalOpen} onClose={handleClose}>
            <div className="text-2xl text-center font-bold pb-6">Group Setting</div>
            <Form {...form}>
